...
//    private  final int ut_timeout = BusinessConstant.WSEB_USER_UT_TIMEOUT; // 一天
...
/*                Cookie[] cookies = req.getCookies();
            ...
            t.setUser(userCache);
        	/*if(userCache.getPlatformId()>10000){
        		t.setTimeout(BusinessConstant.APP_USER_UT_TIMEOUT);
        	}else{
        		t.setTimeout(BusinessConstant.WEB_USER_UT_TIMEOUT);
        	}    */    
            //不做平台设置、采取相同操作
            t.setTimeout(BusinessConstant.WEB_APP_USER_UT_TIMEOUT);

            cache.set(ut, (int) t.getTimeout(), t);
            UserCookieVO userCookie = new UserCookieVO();
            userCookie.setUserId(userCache.getId());
            userCookie.setCookieName(userCache.getCookieName());
    ...
    public  CacheUser getUser(String ut) {
    	...
    	try {
			...
			// 时间计算错误, 乘法优于除法.
			long remain = t.getTimeout() - (now - last); // 缓存剩余分钟数
			if (remain <= 0) {
				cache.del(ut);
                return null;
			}
    ...
    /**
     * @param utKey  根据用户信息查询tokenId  
     * 				  	 如果没有该用户的信息返回null
     * 					 如果redis中有根据用户uuid的用户信息，就返回tokenId 也就是ut
     * @return
     */
    public  String getUt(String utKey) {
    	//userUuid-->tiket(超时时间+token)

    	
    	try {
            if (StringUtils.isBlank(utKey)) {
                return null;
            }

            Ticket t = (Ticket) cache.get(utKey);
    ...
    public  boolean deleteCacheUserByUt(String ut) {
    	cache.del(ut);
        return true;
    }
