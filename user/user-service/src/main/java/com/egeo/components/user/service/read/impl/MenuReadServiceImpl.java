...
public class MenuReadServiceImpl implements MenuReadService {
    ...
    public JsonResult<PageResult<Menu>> findWithTx(Pagination page,MenuDTO menuDTO) {
        ...
        for (MenuPO menuPO : list) {
            ...
            menu.setRemarks(menuPO.getRemarks());
            menu.setIndex(menuPO.getHref());
            menu.setIcon(menuPO.getIcon());
            menu.setSysCode(menuPO.getSysCode());
    ...
    public List<Menu> getShowMenuListByUserId(Long UserId,Long platformId) {
        List<MenuPO> list = menuReadManage.getShowMenuListByUserId(UserId,platformId);
        Integer userType = RuntimeContext.cacheUser().getType();
        List<Menu> menuList = new ArrayList<Menu>();
        for (MenuPO menuPO : list) {
            Menu menu = new Menu();
            if(menuPO.getSysCode()!=null) {
            	if(userType.intValue()==1) {
            		if(!menuPO.getSysCode().equalsIgnoreCase("sys_platform")) {
            			continue;
            		}
            	}else if(userType.intValue()==2) {
            		if(!menuPO.getSysCode().equalsIgnoreCase("sys_manage")) {
            			continue;
            		}
    ...
    public List<Menu> getShowMenuListBySysCode(String sysCode,Long platformId) {
        List<MenuPO> list = menuReadManage.getMenuListBySysCode(sysCode,platformId);
