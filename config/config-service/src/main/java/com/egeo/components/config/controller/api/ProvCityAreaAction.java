...
@Controller
@RequestMapping("/api/config/provCityArea")
public class ProvCityAreaAction extends BaseSpringController {
	...
	public JsonResult<ProvCityAreaVO> findProvCityAreaById(Long id ) {
		...
		ProvCityAreaDTO dto = new ProvCityAreaDTO();
		dto.setId(id);
		ProvCityAreaDTO rt = provCityAreaManage.findProvCityAreaById(dto);		
		return success(ProvCityAreaConverter.toVO(rt));
	...
	@ResponseBody
	public JsonResult<PageResult<ProvCityAreaVO>> findProvCityAreaOfPage(ProvCityAreaVO vo,Pagination page,HttpServletRequest req ) {
		ProvCityAreaDTO dto = ProvCityAreaConverter.toDTO(vo);
