...
import com.egeo.components.stock.request.ProductInventoryCacheRefreshRequest;
/*import com.egeo.basics.stock.request.ProductInventoryCacheRefreshRequest;
import com.egeo.basics.stock.request.RequestQueue;*/
import com.egeo.components.stock.request.ProductInventoryDBUpdateRequest;
import com.egeo.components.stock.request.RequestQueue;
...
public class RequestAsyncProcessServiceImpl implements RequestAsyncProcessService {
	...
	public void process(Request request) {
		try {
			...
			if(request instanceof ProductInventoryDBUpdateRequest) {
				...
				flagMap.put(request.getId(), true);
	 ...
	 * @return 内存队列
	 */
	private ArrayBlockingQueue<Request> getRoutingQueue(Long commodityProductUnitId) {
		RequestQueue requestQueue = RequestQueue.getInstance();
		
		// 先获取productId的hash值
		String key = String.valueOf(commodityProductUnitId);
		int h;
		...
		System.out.println("===========日志===========: 路由内存队列，商品id=" + commodityProductUnitId + ", 队列索引=" + index);  
